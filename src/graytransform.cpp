...
GrayTransform::GrayTransform(QImage *const src, QImage *const dst, QObject *parent) :  QObject(parent)
{
    srcImage = src;
    dstImage = dst;
    return;
...
void GrayTransform::convertToGray()
{
    emit print_progress(0);
    emit print_message(QString("applying convertion to Gray..."));
    ...
    *dstImage = QImage(w, h, QImage::Format_RGB32);
    ...
    // convert rgb to yuv
    rgb2gray(src_r, src_g, src_b, dst_gray, w*h);
    ...
    emit image_ready();
    emit print_message(QString("applying convertion to Gray...finished"));
